...
def telegram_bot(token):
    ...
    def help_message(message):
        try:
            if message.from_user.id == adminId and message.chat.id == adminId:
                bot.send_message(message.chat.id,
                                 f"Команды для простых смертных:\n{const.private_commands}\nМои команды:\n{const.admin_commands}")
            ...
            try:
                if message.chat.type == 'private':
                    ...
                    if users:
                        ...
                        bot.send_message(message.chat.id,
                                         "Вы отменили регистрацию на участие🙅‍♂️ и проебали всю статистику\nА что поделать, такова жизнь🤷‍♂️")
                    else:
        ...
        try:
            ...
            try:
                if message.chat.type == 'private':
                    ...
                    if bool(len(users)):
                        info_message = "📋Зарегистрированые участники:\n"
                        i = 1
                        for user in users:
                            info_message += f"{i}.👉 {user[3] if user[3] else user[4]}"
                            i += 1
                        bot.send_message(message.chat.id, info_message)
                    else:
                        bot.send_message(message.chat.id,
                                         "Нет зарегистрированых пользователей🙇‍♂️, чтобы зарегистрироваться напишите 👉/reg@pidorochek_bot")
            except Exception as e:
                print(e)
                error_message(message, bot)
            finally:
            ...
            try:
                if message.chat.type == 'private':
                    ...
                    if user:
                        ...
                        bot.send_message(message.chat.id,
                                         "К сожалению ты не зарегистрирован на участие😔 Зарегистрируйся с помощью команды 👉/reg@pidorochek_bot")
            except Exception as e:
                print(e)
                error_message(message, bot)
            finally:
                    ...
                    if time_cd >= cooldown_time:
                        ...
                        if temp_time == 0:
                            ...
                            return bot.send_message(message.chat.id,
                                                    f"До следующего определения пидора🌈 осталось {temp_time} минут(ы)⏳")
                        elif temp_time == 21 or temp_time == 1:
                            return bot.send_message(message.chat.id,
                                                    f"До следующего определения пидора🌈 остался {temp_time} час⏳")
                        elif temp_time == 2 or temp_time == 3 or temp_time == 4 or temp_time == 22 or temp_time == 23:
                            return bot.send_message(message.chat.id,
                                                    f"До следующего определения пидора🌈 осталось {temp_time} часa⏳")
                        else:
                            return bot.send_message(message.chat.id,
                                                    f"До следующего определения пидора🌈 осталось {temp_time} часов⏳")
                if message.chat.type == 'private':
                    return wrong_chat_message(message, bot)
                else:
    ...
    def update_data_message(message):
        try:
            psql = PostgreSQL(DATABASE, USER, PASSWORD, HOST, PORT)
            try:
                psql.update_data(message.from_user.id, str(message.from_user.username),
            ...
            try:
                if message.from_user.id == adminId:
                    ...
                    try:
                        ...
                        return bot.send_message(message.chat.id,
                                                "Чето ты не правильно ввел, перепроверь написание команды!\nДолжно быть так /changecooldowntime <секунды>")
                    ...
                    for ids in chat_ids:
                        try:
                            bot.send_message(ids[0],
                                             f"⌛️Кулдаун на определение пидораса был изменен\n\n🆕Теперь {new_cd_time}c")
